...
<html>
	<head>
    	...
    	<title>今日评论-<%=getServletContext().getServletContextName()%></title>
	...
	<body>
		...
		<div id="page">
			...
			<div id="content">
				<div id="left">
					<ul id="left_nav">
						<li id="current_left_nav"><a href="todaycomments.action">今日评论</a></li>
				...
				<div id="right">
					...
					<div id="paginating">
					  <c:if test="${requestScope.page_index > 1 }">
						<span><a href="todaycomments.action?page_index=${requestScope.page_index - 1 }">上一页</a></span>
					  </c:if>
					  <c:if test="${requestScope.page_index <= 1 }">
						<span>上一页</span>
					  </c:if>
					  <a href="todaycomments.action?page_index=${requestScope.page_index }">
						${requestScope.page_index }
					  </a>
					  <c:if test="${requestScope.page_index < requestScope.comments.allPages }">
						<span><a href="todaycomments.action?page_index=${requestScope.page_index + 1 }">下一页</a></span>
					  </c:if>
					  <c:if test="${requestScope.page_index >= requestScope.comments.allPages }">
						<span>下一页</span>
					  </c:if>
