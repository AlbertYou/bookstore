...
<html>
	<head>
    	...
    	<title>低分评论-<%=getServletContext().getServletContextName()%></title>
			...
			<div id="content">
				<div id="left">
					<ul id="left_nav">
						...
						<li id="current_left_nav"><a href="lowscorecomments.action">低分评论</a></li>
						<li><a href="querycomments.action">所有评论</a></li>
					 </ul>
				</div>
				<div id="right">
					<div id="temp_nav">
					...
					<div id="paginating">
					  <c:if test="${requestScope.page_index > 1 }">
						<span><a href="lowscorecomments.action?page_index=${requestScope.page_index - 1 }">上一页</a></span>
					  </c:if>
					  <c:if test="${requestScope.page_index <= 1 }">
						<span>上一页</span>
					  </c:if>
					  <a href="lowscorecomments.action?page_index=${requestScope.page_index }">
						${requestScope.page_index }
					  </a>
					  <c:if test="${requestScope.page_index < requestScope.comments.allPages }">
						<span><a href="lowscorecomments.action?page_index=${requestScope.page_index + 1 }">下一页</a></span>
					  </c:if>
					  <c:if test="${requestScope.page_index >= requestScope.comments.allPages }">
						<span>下一页</span>
					  </c:if>
