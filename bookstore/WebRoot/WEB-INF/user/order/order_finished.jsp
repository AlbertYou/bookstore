...
<html>
	<head>
    	...
    	<title>已完成的交易-<%=getServletContext().getServletContextName()%></title>
			...
			<div id="content">
			  <div id="left">
					<div id="nav_panel">
						...
						<div class="current_nav"><a href="finishedorderlist.action">交易成功</a></div>
              ...
              <div id="right">
				<div id="temp_nav">
					<span id="order_total">已完成交易(<s:property value="#request.orders.list.size"/>笔)</span>
					<span id="order_sort">按购买时间倒序排列</span>
				</div>
				<div id="orders">
					<ul id="orderlist">
				...
				<div id="paginating">
					<c:if test="${requestScope.page_index > 1 }">
						<span><a href="finishedorderlist.action?page_index=${requestScope.page_index - 1 }">上一页</a></span>
					</c:if>
					<c:if test="${requestScope.page_index <= 1 }">
						<span>上一页</span>
					</c:if>
					<a href="finishedorderlist.action?page_index=${requestScope.page_index }">
						${requestScope.page_index }
					</a>
					<c:if test="${requestScope.page_index < requestScope.orders.allPages }">
						<span><a href="finishedorderlist.action?page_index=${requestScope.page_index + 1 }">下一页</a></span>
					</c:if>
					<c:if test="${requestScope.page_index >= requestScope.orders.allPages }">
						<span>下一页</span>
					</c:if>
